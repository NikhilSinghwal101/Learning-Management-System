...
// Based on life-time access not subscription based
export const userStats = asyncHandler(async (req, res, next) => {
  const allUsersCount = await User.countDocuments();

  const purchasedUsersCount = await User.countDocuments({
  ...
  res.status(200).json({
    ...
    allUsersCount,
    purchasedUsersCount, // renamed from subscribedUsersCount
  });
});
